...
function max_path_through_pyramid(pyramid::Vector{Vector{Integer}}=Vector{Vector{Integer}}([Vector{Integer}([1])]))::Integer
    ...
    for i = 1:length(pyramid)
        ...
        if i == 1
            @inbounds pyramid_best_paths[i][1] = pyramid[i][1]
        else
            @inbounds pyramid_best_paths[i][1] = pyramid_best_paths[i-1][1] + pyramid[i][1]
            @inbounds pyramid_best_paths[i][end] = pyramid_best_paths[i-1][end] + pyramid[i][end]
        end
    end

    for i = 3:length(pyramid)
        for j = 2:length(pyramid[i])-1
            @inbounds pyramid_best_paths[i][j] = max(pyramid_best_paths[i-1][j-1],
