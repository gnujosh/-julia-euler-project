...
# Integer values don't promote to BigInt, so assume with BigInt.
function p011solution(matrix::Array{BigInt, 2}, adjacent::Integer)::BigInt
    ...
    # Check \ diagonals
    for row = 1:nrows-adjacent+1
        for col = 1:ncols-adjacent+1
...
p011 = Problems.Problem(p011solution)
...
Problems.benchmark(p011, convert(Array{BigInt}, input), 4)